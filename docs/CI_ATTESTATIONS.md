...
revision, ref, run, and attempt. The verifier requires the complete eleven-package
Linux/BSD set: Debian and RPM on Linux amd64 and arm64, FreeBSD/OpenBSD/NetBSD
packages on amd64 and arm64, and DragonFly BSD on amd64. The Linux package job
uses native amd64 and arm64 hosted runners; BSD package jobs are explicitly
virtualized. Package subjects are
...
The non-PR CI workflow now runs reference builders for all eleven Linux/BSD
target/architecture mappings: Debian/RPM on native hosted Linux amd64 and
arm64 runners, FreeBSD/OpenBSD/NetBSD package tools in virtualized amd64 and
arm64 BSD guests, and DragonFly package tools in a virtualized amd64 guest.
They install and uninstall the
