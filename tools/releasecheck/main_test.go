...
func TestCheckReleaseAcceptsCanonicalRelease(t *testing.T) {
	...
	if err := checkRelease(dir, testVersion, testEpoch, commit, tree, testBuilderGoVersion); err != nil {
...
func TestCheckReleaseRejectsChecksumMismatch(t *testing.T) {
	...
	if err := checkRelease(dir, testVersion, testEpoch, testCommit, testTree, testBuilderGoVersion); err == nil || !strings.Contains(err.Error(), "checksum mismatch") {
...
func TestReleaseBinaryBindsTargetAndIdentity(t *testing.T) {
	...
	if err := checkReleaseBinary(binary, linux, testVersion, testEpoch, commit, tree, testBuilderGoVersion); err != nil {
		t.Fatalf("checkReleaseBinary() error = %v", err)
	}
	if err := checkReleaseBinary(binary, linux, testVersion, testEpoch, commit, tree, "go1.99.0"); err == nil || !strings.Contains(err.Error(), "Go builder version") {
		t.Fatalf("wrong-builder error = %v; want Go builder version rejection", err)
	}
	if err := checkReleaseBinary(binary, linux, testVersion, testEpoch+2, commit, tree, testBuilderGoVersion); err == nil || !strings.Contains(err.Error(), "structured Go build ID") {
		t.Fatalf("wrong-epoch build ID error = %v; want structured-contract rejection", err)
	}
	if err := checkReleaseBinary(binary, linux, testVersion, testEpoch, commit, testTree, testBuilderGoVersion); err == nil || !strings.Contains(err.Error(), "structured Go build ID") {
		t.Fatalf("wrong-tree build ID error = %v; want source-tree contract rejection", err)
	}

	freeBSD := linux
	freeBSD.goos = "freebsd"
	if err := checkReleaseBinary(binary, freeBSD, testVersion, testEpoch, commit, tree, testBuilderGoVersion); err == nil || !strings.Contains(err.Error(), "ELF OSABI") {
	...
	if err := checkReleaseBinary(missingIdentity, linux, testVersion, testEpoch, commit, tree, testBuilderGoVersion); err == nil || !strings.Contains(err.Error(), "occurs 0 times") {
		t.Fatalf("missing identity error = %v", err)
	}

	duplicateIdentity := append(append([]byte(nil), binary...), identity...)
	if err := checkReleaseBinary(duplicateIdentity, linux, testVersion, testEpoch, commit, tree, testBuilderGoVersion); err == nil || !strings.Contains(err.Error(), "occurs 2 times") {
	...
	if err := checkReleaseBinary(missingVerification, linux, testVersion, testEpoch, commit, tree, testBuilderGoVersion); err == nil || !strings.Contains(err.Error(), "repository evidence verification occurs 0 times") {
	...
	if err := checkReleaseBinary(mutatedScorecard, linux, testVersion, testEpoch, commit, tree, testBuilderGoVersion); err == nil || !strings.Contains(err.Error(), "embedded readiness scorecard occurs 0 times") {
	...
	if err := checkReleaseBinary(vcsBinary, linux, testVersion, testEpoch, commit, tree, testBuilderGoVersion); err == nil || !strings.Contains(err.Error(), "unexpected Go build setting") {
...
func TestReleaseBinaryRejectsContainerIdentityMutations(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			err := checkReleaseBinary(mutated, test.item, testVersion, testEpoch, commit, tree, testBuilderGoVersion)
			if err == nil || !strings.Contains(err.Error(), test.want) {
...
func TestReleaseBinaryRejectsAdditionalCompiledModule(t *testing.T) {
	...
	if err := checkReleaseBinary(binary, item, testVersion, testEpoch, commit, tree, testBuilderGoVersion); err == nil || !strings.Contains(err.Error(), "compiled dependency modules") {
...
func buildTestBinaryMode(t *testing.T, repository, commit string, item artifact, trimpath, stripped, buildVCS bool, identity string) []byte {
	...
	ldflagParts := []string{
		...
		"-X", mainModulePath + "/internal/version.RepositoryEvidenceVerification=" + readiness.ExpectedRepositoryEvidenceVerification(),
	}
	if stripped {
