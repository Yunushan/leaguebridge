// Package ciattestation generates strict, score-free subjects for CI artifact
...
// GenerateRequest supplies explicit source and execution metadata for an unsigned
// CI attestation subject. Relative subject paths resolve from the working directory.
...
// VerifyRequest identifies a complete CI or release artifact set and the source
// identity expected by the caller. GHPath selects the trusted GitHub CLI executable;
// no repository, workflow, revision, or run identity is inferred by the verifier.
...
// GenerateFile validates the supplied metadata and creates a new unsigned JSON
// subject at path. It refuses to overwrite an existing output. Generation does not
// authenticate execution or establish eligibility for readiness points.
func GenerateFile(path string, input GenerateRequest) error {
	value, err := build(input)
	if err != nil {
		return fmt.Errorf("build CI attestation subject: %w", err)
	}
	data, err := marshal(value)
	if err != nil {
		return fmt.Errorf("marshal CI attestation subject: %w", err)
	}
	if err := writeNew(path, data); err != nil {
		return fmt.Errorf("write CI attestation subject: %w", err)
	}
	return nil
}

func build(input GenerateRequest) (document, error) {
...
// VerifySet verifies a complete CI or release artifact set against the caller's
// expected identity. It invokes GitHub CLI for every subject and never treats a
// copied subject JSON file as authentication. Success means only that these
...
// A named buffer prevents io.Copy/os/exec from bypassing Write via ReaderFrom.
