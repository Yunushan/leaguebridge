// Package releasecheck validates the complete layout and integrity of a
// LeagueBridge release directory against independently selected source identity.
...
// CheckRequest supplies the exact release identity and expected scorecard bytes.
// ExpectedScorecard must already be authenticated against the selected source by
// the caller. Check binds those bytes and their existing build-time marker to
// every binary; it does not authenticate a publisher or award readiness points.
...
// CLI uses it to preserve its existing distinction between usage and file errors.
func (input CheckRequest) ValidateIdentity() error {
	if _, err := parseSourceDateEpoch(strconv.FormatInt(input.SourceDateEpoch, 10)); err != nil {
		return err
	}
	if err := validateCommit(input.Commit); err != nil {
		return err
	}
	if err := validateTree(input.Tree); err != nil {
		return err
	}
	return validateProductionBuilderGoVersion(input.BuilderGoVersion)
}

// ParseSourceDateEpoch preserves the command's bounded decimal timestamp syntax.
func ParseSourceDateEpoch(raw string) (int64, error) { return parseSourceDateEpoch(raw) }

func scorecardVerification(scorecard []byte) string {
	digest := sha256.Sum256(scorecard)
	return readiness.RepositoryEvidenceVerificationPrefix + hex.EncodeToString(digest[:])
}
...
func checkArchivePayload(payload archivePayload, item artifact, version string, epoch int64, commit, tree, builderGoVersion string, expectedScorecard []byte) error {
	...
	if err := checkReleaseBinary(payload.binary, item, version, epoch, commit, tree, builderGoVersion, expectedScorecard); err != nil {
...
func checkReleaseBinary(binaryData []byte, item artifact, version string, epoch int64, commit, tree, builderGoVersion string, expectedScorecard []byte) error {
	...
	embeddedScorecard := expectedScorecard
	...
	repositoryEvidenceVerification := scorecardVerification(expectedScorecard)
