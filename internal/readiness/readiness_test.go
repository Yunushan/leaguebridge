...
var readinessTestNow = time.Date(2026, time.September, 4, 12, 0, 0, 0, time.UTC)
...
func TestValidityAndParserBoundsFailClosed(t *testing.T) {
	tests := []struct {
		...
		{name: "expired", data: embedded, now: time.Date(2026, time.October, 4, 0, 0, 1, 0, time.UTC)},
		{name: "validity over maximum", data: mutateEmbeddedDocument(t, func(document map[string]any) {
			document["expires_at"] = time.Date(2026, time.October, 5, 0, 0, 1, 0, time.UTC).Format(time.RFC3339Nano)
		}), now: readinessTestNow},
