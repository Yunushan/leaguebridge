...
func TestValidityAndParserBoundsFailClosed(t *testing.T) {
	tests := []struct {
		...
		{name: "expired", data: embedded, now: readinessTestNow.Add(MaximumValidity + time.Second)},
		{name: "validity over maximum", data: mutateEmbeddedDocument(t, func(document map[string]any) {
			assessed, err := time.Parse(time.RFC3339, document["assessed_at"].(string))
			if err != nil {
				t.Fatal(err)
			}
			document["expires_at"] = assessed.Add(MaximumValidity + time.Second).Format(time.RFC3339Nano)
		}), now: readinessTestNow},
