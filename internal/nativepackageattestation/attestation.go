// Package nativepackageattestation builds and verifies score-free GitHub
// attestations for native package CI output. Verification does not establish
// production package signing, publication, or independent native install.
...
// BuildRequest describes the CI subject to be generated from local package,
// staging, and install-evidence files.
...
// VerifyRequest binds the complete subject set to an independently chosen
// source checkout and GitHub Actions run. Subject JSON cannot select these.
...
// VerifiedPackage is a verified CI package artifact. Its digest is checked
// against the subject bytes and a GitHub artifact attestation. Production
// signatures and publication are outside this verifier's scope.
...
// VerifiedSet can only be obtained from a successful Verify call. Its zero
// value is invalid, so callers cannot promote a missing verifier result.
...
// Valid reports whether the complete set was successfully verified.
func (set VerifiedSet) Valid() bool { return set.verified }

// Version returns the common package version, or an empty string for an
// invalid set.
func (set VerifiedSet) Version() string {
	if !set.verified {
		return ""
	}
	return set.version
...
// Packages returns an independent copy of the complete verified inventory.
func (set VerifiedSet) Packages() []VerifiedPackage {
	if !set.verified {
		return nil
	}
	return append([]VerifiedPackage(nil), set.packages...)
}
...
// Build inventories a staged native package and its bounded install log.
func Build(input BuildRequest) (document, error) { return build(input) }

// Marshal encodes a validated subject in the canonical schema form.
func Marshal(value document) ([]byte, error) { return marshal(value) }

// WriteNew creates a subject file without replacing an existing path.
func WriteNew(path string, data []byte) error { return writeNew(path, data) }

// NormalizeRelativePath applies the subject path policy used by Build and
// Verify. The CLI uses it before writing a new document.
...
func Verify(ctx context.Context, input VerifyRequest) (VerifiedSet, error) {
	...
	return verifySet(ctx, input)
}

func build(input BuildRequest) (document, error) {
...
func verifySet(ctx context.Context, input VerifyRequest) (VerifiedSet, error) {
	...
	if input.GHPath == "" {
		return VerifiedSet{}, errors.New("GitHub CLI path is required")
	}
	if len(input.SubjectPaths) == 0 {
		return VerifiedSet{}, errors.New("at least one -verify-subject is required")
	}
	if !validRepository(input.ExpectedRepo) {
		return VerifiedSet{}, errors.New("expected repository is invalid")
	}
	...
	if err != nil || workflowPath != filepath.ToSlash(input.ExpectedWorkflow) || workflowPath != defaultWorkflow {
		return VerifiedSet{}, errors.New("expected workflow path is invalid or not the CI workflow")
	}
	if !objectIDPattern.MatchString(input.ExpectedCommit) || !objectIDPattern.MatchString(input.ExpectedTree) || !objectIDPattern.MatchString(input.WorkflowSHA) {
		return VerifiedSet{}, errors.New("expected commit, tree, and workflow SHA must be lowercase 40- or 64-character object IDs")
	}
	if !refPattern.MatchString(input.ExpectedRef) || !runIDPattern.MatchString(input.RunID) || !attemptPattern.MatchString(input.RunAttempt) {
		return VerifiedSet{}, errors.New("expected ref, run ID, or run attempt is invalid")
	}
	...
	for _, path := range input.SubjectPaths {
		...
		if _, duplicate := seenPaths[path]; duplicate {
			return VerifiedSet{}, fmt.Errorf("verification subject %q is duplicated", path)
		}
		...
		if err := validateExpectedSource(item.Value, input, workflowPath); err != nil {
			return VerifiedSet{}, fmt.Errorf("subject %q: %w", path, err)
		}
	...
	packages := make([]VerifiedPackage, 0, len(loaded))
	for _, item := range loaded {
		if err := ctx.Err(); err != nil {
			return VerifiedSet{}, err
		}
		if err := verifyStagingDocument(ctx, item.Value); err != nil {
			return VerifiedSet{}, fmt.Errorf("subject %q staging: %w", item.Path, err)
		}
		...
		if err := validateInstallEvidence(artifact.InstallEvidencePath, artifact.Family, artifact.Version, artifact.Filename, item.Value.Execution.Target); err != nil {
			return VerifiedSet{}, fmt.Errorf("subject %q install evidence: %w", item.Path, err)
		}
		...
		for _, declared := range item.Value.Subjects {
			...
			if _, duplicate := seenArtifacts[declared.Path]; duplicate {
				return VerifiedSet{}, fmt.Errorf("package artifact %q is listed more than once", declared.Path)
			}
			seenArtifacts[declared.Path] = struct{}{}
			actual, err := hashPathContext(ctx, declared.Path, declared.Role)
			if err != nil {
				return VerifiedSet{}, fmt.Errorf("subject %q artifact %q: %w", item.Path, declared.Path, err)
			}
			if actual.SizeBytes != declared.SizeBytes || actual.SHA256 != declared.SHA256 {
				return VerifiedSet{}, fmt.Errorf("subject %q artifact %q does not match its declared size or SHA-256", item.Path, declared.Path)
			}
		...
		for _, artifact := range artifacts {
			if err := verifyGitHubArtifact(ctx, artifact, commonSource, input.GHPath); err != nil {
				return VerifiedSet{}, fmt.Errorf("subject %q artifact %q: %w", item.Path, artifact.Path, err)
			}
		}
		packages = append(packages, VerifiedPackage{
			Family: nativepackage.Family(artifact.Family),
			GOOS:   item.Value.Execution.Target.GOOS, GOARCH: item.Value.Execution.Target.GOARCH,
			Format: artifact.Format, Version: artifact.Version, Filename: artifact.Filename,
			SubjectPath:           item.Path,
			SubjectSHA256:         digestBytes(item.Data),
			Path:                  findRolePath(item.Value.Subjects, "package"),
			SHA256:                findRoleDigest(item.Value.Subjects, "package"),
			StagingManifestSHA256: findRoleDigest(item.Value.Subjects, "staging-manifest"),
			InstallEvidenceSHA256: findRoleDigest(item.Value.Subjects, "package-install-evidence"),
		})
	}
	if err := ctx.Err(); err != nil {
		return VerifiedSet{}, err
	...
	sort.Slice(packages, func(i, j int) bool {
		if packages[i].Family != packages[j].Family {
			return packages[i].Family < packages[j].Family
		}
		if packages[i].GOOS != packages[j].GOOS {
			return packages[i].GOOS < packages[j].GOOS
		}
		return packages[i].GOARCH < packages[j].GOARCH
	})
	return VerifiedSet{verified: true, version: packages[0].Version, packages: packages}, nil
}

func verifyStagingDocument(ctx context.Context, value document) error {
	...
	manifest, verifyErr := nativepackage.VerifyStagingRootContext(ctx, root)
	closeErr := root.Close()
	if verifyErr != nil {
		return verifyErr
	}
...
func findRoleDigest(subjects []subject, role string) string {
	for _, item := range subjects {
		if item.Role == role {
			return item.SHA256
		}
